...
if ($flag == 1) {
    ...
    $pagecount = $pdf->setSourceFile($uploadFilePath);


    for($i=1;$i<=$pagecount;$i=$i+2){
...
// echo "sdfsdf";
        ...
        if($i+2<=$pagecount){
            $i=$i+2;
            $templateId=$pdf->importPage($i);
            $pdf->useTemplate($templateId,100,0,$newWidth,$newHeight);
        ...
        if($i+2<=$pagecount){
            $i=$i+2;
            $templateId=$pdf->importPage($i);
            $pdf->useTemplate($templateId,0,143,$newWidth,$newHeight);
        ...
        if($i+2<=$pagecount){
            $i=$i+2;
            $templateId=$pdf->importPage($i);
            $pdf->useTemplate($templateId,100,143,$newWidth,$newHeight);
    ...
    $new_filename = $i . 'amazon.pdf';
    $pdf->Output('../split/'.$new_filename, "F");

    // http://localhost/InternShip/Amazon/4Pages/split/7amazon.pdf
    echo json_encode(['filePath' => 'http://localhost/InternShip/Amazon/4Pages/split/7amazon.pdf']);
